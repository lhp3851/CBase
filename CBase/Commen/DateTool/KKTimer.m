...
-(void)getServerTime
{
    NSString *url=[AbsoluntRequestUrl reqestWithStarService:kUSER_SERVER_TIME_API];
    __block NSTimeInterval startTime = [[NSDate date] timeIntervalSince1970];
    __weak __typeof(self)weakSelf = self;
    [KKHttpTool get:url params:nil cookied:NO needLogin:NO success:^(id json) {
        NSNumber *retCode=json[@"rtn"];
        Print(@"KKLive_QueryGifts retCode = %ld", retCode.integerValue);
...
-(void)timerCrile:(NSTimer *)timer
{
    ...
    dispatch_async(_timerQueue, ^{
        for (int i=0; i<_timerArray.count; i++) {
            ...
            info.lastTime = now;
            //            Print(@"----%f",info.end - info.lastTime);
