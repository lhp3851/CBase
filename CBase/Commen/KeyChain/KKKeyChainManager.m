...
+ (NSString *)getUUIDString
{
    ...
    if (!uuid || !uuid.length ) {
        ...
        if (uuid && uuid.length) {
            ...
            NSString * rawKey   = [kUSER_RANDOM_KYE md5Encrypt];
            NSString * sign     = [md5DeviceID stringByAppendingString:kBundle_Name];
            NSString * sign1    = [sign stringByAppendingFormat:@"%d%@",kTONG_MENG_NET_BUSINESS_TYPE_iPHONE,rawKey];
