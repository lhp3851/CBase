...
- (NSString *)emojiCodeToStringCode
{
    NSString *hex = [NSString hexStringFromString:self];
    return hex;
}
- (NSString *)stringCodeToEmojiCode
{
    NSString *emoji = [NSString stringFromHexString:self];
    return emoji;
}
// 判断是否是 emoji表情
...
- (BOOL)isEmoji
{
    ...
    if (0xD800 <= high && high <= 0xDBFF) {
        ...
        re = (0x1D000 <= codepoint && codepoint <= 0x1F9FF);
        
        // Not surrogate pair (U+2100-27BF)
    } else {
        re = (0x2100 <= high && high <= 0x27BF);
    }
    return re;
}
/**
 *  表情unicodeString->表情unicode
 *
...
+ (NSString *)translateTextToEmojiString:(NSString *)text
{
    ...
    NSString *emojiPattern = @"<e>*>.*?<\\/e>";
    __block NSMutableString *translatedString = [text mutableCopy];
    [text enumerateStringsMatchedByRegex:emojiPattern usingBlock:^(NSInteger captureCount, NSString *const __unsafe_unretained *capturedStrings, const NSRange *capturedRanges, volatile BOOL *const stop) {
        translatedString = [[text substringToIndex:capturedRanges->location] mutableCopy];
        NSString *emojiConstString = *capturedStrings;
        NSRange emojiRange;
        emojiRange.location = 3;
        emojiRange.length = capturedRanges->length - 7;
        NSString *emojiCode = [emojiConstString substringWithRange:emojiRange];
        NSString *emoji;
        if (emojiCode.length) {
            emoji = [emojiCode isEmojiWithReturnUnicode];//[NSString stringFromHexString:emojiCode];
        }
        if (emoji) {
            [translatedString appendString:emoji];
        }
        *stop = YES;
        NSString *unTranslatedString = [text substringFromIndex:capturedRanges->location + capturedRanges->length];
        if (unTranslatedString.length) {
            [translatedString appendString:[NSString translateTextToEmojiString:unTranslatedString]];
        }
    }];
    return translatedString;
}
/**
 *  表情unicode->表情unicodeString
 *
 *  @return <#return value description#>
 */
+ (NSString *)translateTextToEmojiCodeString:(NSString *)text
{
    ...
    __block NSMutableString *translatedString = [NSMutableString string];
    [text enumerateSubstringsInRange:NSMakeRange(0, text.length) options:NSStringEnumerationByComposedCharacterSequences usingBlock:^(NSString *substring, NSRange substringRange, NSRange enclosingRange, BOOL *stop) {
        translatedString = [[text substringToIndex:substringRange.location] mutableCopy];
        NSString *emojiCodeString = [substring iSEmojiWithReturnCodeString];
        if (emojiCodeString) {//替换成
            *stop = YES;
            [translatedString appendString:@"<e>"];
            [translatedString appendString:emojiCodeString];//[NSString hexStringFromString:substring]];
            [translatedString appendString:@"</e>"];
            NSString *uTranslateString = [text substringFromIndex:substringRange.location + substringRange.length];
            if (uTranslateString.length > 0) {
                [translatedString appendString:[NSString translateTextToEmojiCodeString:uTranslateString]];
            }
        }
        else
...
+ (NSString *)stringFromHexString:(NSString *)hexString { //
    ...
    return [hexString toOriginalString];
}
...
{
    NSString *emojiCode;
    if ([self isEmoji]) {   //是否是表情
        NSDictionary *emojiCodeDict = [NSString emojiCodeDict];  //中转库中
        emojiCode = [emojiCodeDict valueForKey:self];
        if (!emojiCode) {
            emojiCode = [self toHexString];  //转16进制字符串
        }
    }else
        emojiCode = nil;
    return emojiCode;
}
- (NSString *)isEmojiWithReturnUnicode
{
    NSString *originalS = [[NSString emojiAliases] valueForKey:self];
    if (!originalS) {
        originalS = [self toOriginalString];
    }
    return originalS;
}
- (BOOL)containsEmoji
...
{
    static NSDictionary *_emojiCode;
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
        _emojiCode = [NSDictionary dictionaryWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"Emoji.plist" ofType:nil]];
    });
    return _emojiCode;
}
@end
