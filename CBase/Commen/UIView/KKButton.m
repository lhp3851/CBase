...
+(KKButton *)getButtonWithButtonType:(TerminalOperationButton)type{
    KKButton *button =[[KKButton alloc]init];
    button.layer.masksToBounds =YES;
    button.layer.cornerRadius = 5;
    button.titleLabel.font = kFONT_12;
    switch (type) {
        case TerminalOperationButtonLock:
        {
            button.titleLabel.textColor = kBACKGROUND_COLOR;
            [button setTitle:@"锁定" forState:UIControlStateNormal];
            [button setBackgroundColor:kWARNING_COLOR];
        }
            break;
        case TerminalOperationButtonUnLock:
        {
            button.titleLabel.textColor = kBACKGROUND_COLOR;
            [button setTitle:@"解锁" forState:UIControlStateNormal];
            [button setBackgroundColor:kSECURITY_COLOR];
        }
            break;
        case TerminalOperationButtonChangeTicket:
        {
            [button setBackgroundColor:kOPERATE_BACKGROUND_COLOR];
            button.layer.borderWidth = 1.0f;
            button.layer.borderColor = kFOURTH_LEVEL_COLOR.CGColor;
            [button setTitle:@"换票" forState:UIControlStateNormal];
            [button setTitleColor:kTERMINAL_GRID_NAME_COLOR forState:UIControlStateNormal];
            [button setBackgroundImage:[UIImage imageWithColor:kOPERATE_BACKGROUND_HELIGHT_COLOR] forState:UIControlStateHighlighted];
        }
            break;
        case TerminalOperationButtonAddTicket:
        {
            [button setBackgroundColor:kOPERATE_BACKGROUND_COLOR];
            button.layer.borderWidth = 1.0f;
            button.layer.borderColor = kFOURTH_LEVEL_COLOR.CGColor;
            [button setTitle:@"加票" forState:UIControlStateNormal];
            [button setTitleColor:kTERMINAL_GRID_NAME_COLOR forState:UIControlStateNormal];
            [button setBackgroundImage:[UIImage imageWithColor:kOPERATE_BACKGROUND_HELIGHT_COLOR] forState:UIControlStateHighlighted];
        }
            break;
        case TerminalOperationButtondelTicket:
        {
            [button setBackgroundColor:kOPERATE_BACKGROUND_COLOR];
            button.layer.borderWidth = 1.0f;
            button.layer.borderColor = kFOURTH_LEVEL_COLOR.CGColor;
            [button setTitle:@"减票" forState:UIControlStateNormal];
            [button setTitleColor:kTERMINAL_GRID_NAME_COLOR forState:UIControlStateNormal];
            [button setBackgroundImage:[UIImage imageWithColor:kOPERATE_BACKGROUND_HELIGHT_COLOR] forState:UIControlStateHighlighted];
        }
            break;
...
-(instancetype)initWithFrame:(CGRect)Frame SelectedImage:(NSString *)selectedImage NormalImage:(NSString *)normalImage{
    ...
    [checkbox setImage:[FileOperationManager imageWithPath:normalImage] forState:UIControlStateNormal];
    [checkbox setImage:[FileOperationManager imageWithPath:selectedImage] forState:UIControlStateSelected];
    return checkbox;
}
...
-(instancetype)initWithFrame:(CGRect)frame  title:(NSString *)title subTitle:(NSString *)subTitle font:(UIFont *)font subTitleFont:(UIFont *)subFont norTitlecolor:(UIColor *)norcolor hlightTitlecolor:(UIColor *)hightcolor subTitleColor:(UIColor *)subTitleColor  normalBackgroudColor:(UIColor *)normalBackgroudColor HilighBackgroudColor:(UIColor *)HilighBackgroudColor cornerRadius:(CGFloat)cornerRadius borderWidth:(CGFloat)borderWidth borderColor:(UIColor *)borderColor{
    if (self == [super initWithFrame:frame]) {
        ...
        [self setTitleEdgeInsets:UIEdgeInsetsMake(0, 0, kFitWithHeight(frame.size.height *0.1), 0)];
        
        CGPoint subPoint = {0,frame.size.height/2 - kFitWithHeight(frame.size.height *0.05)};
        CGSize subSize   = {frame.size.width,frame.size.height/2};
        CGRect subFrame = {subPoint,subSize};
        KKLabel *subTitleLab = [[KKLabel alloc]initWithFrame:subFrame text:subTitle color:subTitleColor font:subFont alignment:NSTextAlignmentCenter];
        ...
        [self sendSubviewToBack:subTitleLab];
    }
    return self;
}
