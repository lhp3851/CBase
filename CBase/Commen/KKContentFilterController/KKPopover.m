...
@property (nonatomic, assign, readwrite) KKPopoverPosition popoverPosition;
...
@implementation KKPopover {
    BOOL _isiOS7;
    BOOL _setNeedsReset;
}

+ (instancetype)popover {
    return [[KKPopover alloc] init];
}
...
- (void)commonInit {
    ...
    self.sideEdge = 10.0;
    self.maskType = KKPopoverMaskTypeBlack;
...
- (void)setBackgroundColor:(UIColor *)backgroundColor {
    [super setBackgroundColor:kCLEAR_COLOR];
...
- (void)showAtPoint:(CGPoint)point
     popoverPostion:(KKPopoverPosition)position
    ...
    switch (self.maskType) {
        case KKPopoverMaskTypeBlack:
            maskColor = [UIColor colorWithWhite:0.0 alpha:0.3];
            break;
        case KKPopoverMaskTypeNone: {
            maskColor = kCLEAR_COLOR;
    ...
    self.blackOverlay.backgroundColor = kCLEAR_COLOR;
...
- (void)showAtView:(UIView *)atView
   ...
   withContentView:(UIView *)contentView
    ...
    KKPopoverPosition popoverPosition;
    if (upCanContain) {
        popoverPosition = KKPopoverPositionUp;
        atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
    } else {
        popoverPosition = KKPopoverPositionDown;
    ...
    if (upCanContain && downCanContain) {
        ...
        if (position != 0) {
            useUp = position == KKPopoverPositionUp ? YES : NO;
        }
        if (useUp) {
            popoverPosition = KKPopoverPositionUp;
            atPoint.y = CGRectGetMinY(atViewFrame) - betweenArrowAndAtView;
...
- (void)show {
    ...
    CGFloat originY = 0.0;
    if (self.popoverPosition == KKPopoverPositionDown) {
...
- (void)showAtView:(UIView *)atView withText:(NSAttributedString *)abs inView:(UIView *)container {
    ...
    [textLabel sizeToFit];
    textLabel.backgroundColor = kCLEAR_COLOR;
